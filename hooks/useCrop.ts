...
import { CropState } from "@/types/camera"
      ...
      if (cropState.isDragging) {
        setCropState((prev) => ({
          ...
          cropArea: {
            ...
            x: Math.max(
              0,
              Math.min(1 - prev.cropArea.width, prev.cropArea.x + deltaX)
            ),
            y: Math.max(
              0,
              Math.min(1 - prev.cropArea.height, prev.cropArea.y + deltaY)
            ),
          },
          dragStart: { x: e.clientX, y: e.clientY },
        }))
          ...
          switch (prev.resizeHandle) {
            ...
            case "ne":
              newArea.width = Math.max(
                0.1,
                Math.min(1 - prev.cropArea.x, prev.cropArea.width + deltaX)
              )
              newArea.height = Math.max(0.1, prev.cropArea.height - deltaY)
              newArea.y = Math.max(0, prev.cropArea.y + deltaY)
              break
            case "sw":
              ...
              newArea.height = Math.max(
                0.1,
                Math.min(1 - prev.cropArea.y, prev.cropArea.height + deltaY)
              )
              newArea.x = Math.max(0, prev.cropArea.x + deltaX)
              break
            case "se":
