...
import { EffectState, VideoEffect } from "@/types/camera"
...
export const useEffects = () => {
  ...
  const toggleEffectCropMode = useCallback(() => {
    setEffectState((prev) => ({
      ...
      effectCropArea: !prev.isEffectCropMode
  ...
  const handleEffectCropMouseMove = useCallback(
    (e: MouseEvent, videoContainerRef: React.RefObject<HTMLDivElement>) => {
      if (
        !videoContainerRef.current ||
        (!effectState.isEffectDragging && !effectState.isEffectResizing)
      )
        return
        ...
        setEffectState((prev) => ({
          ...
          effectCropArea: {
            ...
            x: Math.max(
              0,
              Math.min(
                1 - prev.effectCropArea.width,
                prev.effectCropArea.x + deltaX
              )
            ),
            y: Math.max(
              0,
              Math.min(
                1 - prev.effectCropArea.height,
                prev.effectCropArea.y + deltaY
              )
            ),
          },
          effectDragStart: { x: e.clientX, y: e.clientY },
          ...
          switch (prev.effectResizeHandle) {
            case "nw":
              ...
              newArea.height = Math.max(
                0.1,
                prev.effectCropArea.height - deltaY
              )
            ...
            case "ne":
              newArea.width = Math.max(
                0.1,
                Math.min(
                  1 - prev.effectCropArea.x,
                  prev.effectCropArea.width + deltaX
                )
              )
              newArea.height = Math.max(
                0.1,
    ...
    [
      effectState.isEffectDragging,
      effectState.isEffectResizing,
      effectState.effectDragStart,
      effectState.effectResizeHandle,
    ]
  )
