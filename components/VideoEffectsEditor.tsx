...
export default function VideoEffectsEditor({
  ...
  const [editorVideoRef, setEditorVideoRef] = useState<HTMLVideoElement | null>(
    null
  )
  ...
  const updateEditorEffectPreview = useCallback(() => {
    if (
      !editorPreviewCanvasRef.current ||
      !editorVideoRef ||
      !editorVideoContainerRef.current
    )
      return
    ...
    const containerRect =
      editorVideoContainerRef.current.getBoundingClientRect()
    ...
    if (isEffectCropMode) {
      ...
      const videoEffectStartX =
        (effectStartX - videoOffsetX) * (videoWidth / displayedVideoWidth)
      const videoEffectStartY =
        (effectStartY - videoOffsetY) * (videoHeight / displayedVideoHeight)
      const videoEffectWidth = effectWidth * (videoWidth / displayedVideoWidth)
      const videoEffectHeight =
        effectHeight * (videoHeight / displayedVideoHeight)
    ...
    // Continue animation
    if (videoEffect !== ("none" as VideoEffect)) {
      editorEffectAnimationRef.current = requestAnimationFrame(
  ...
  const processVideoWithEffects = useCallback(async () => {
    if (!uploadedVideoFile || !editorVideoRef || !editorEffectCanvasRef.current)
      return
        ...
        const processFrame = () => {
          ...
          if (videoEffect !== ("none" as VideoEffect)) {
            if (isEffectCropMode) {
              ...
              const clampedWidth = Math.max(
                1,
                Math.min(effectWidth, videoWidth - clampedX)
              )
              const clampedHeight = Math.max(
                1,
                Math.min(effectHeight, videoHeight - clampedY)
              )
              ...
              if (videoEffect === "blur") {
                ...
                const scaledWidth = Math.max(
                  1,
                  Math.floor(clampedWidth / pixelSize)
                )
            ...
            <CardTitle className="flex items-center gap-2">
              ...
              >
                ...
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M10 11V9m4 2V9m-4 4v2m4-2v2"
                />
              </svg>
              Video Effects Editor
            </CardTitle>
            <p className="text-sm text-slate-600">
              Upload and apply effects to existing videos with live preview
            </p>
          </CardHeader>
          <CardContent>
            <div className="text-center space-y-4">
              ...
              <div className="text-xs text-slate-500">
                ✨ Apply blur, pixelate, and area effects • 🎬 Live preview
                while editing • 📥 Export processed video
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {isVideoEditorMode && uploadedVideoUrl && (
        <Card className="overflow-hidden">
          <CardHeader className="pb-4">
            ...
            <p className="text-sm text-slate-600">
              Apply effects to your video with live preview
            </p>
          </CardHeader>
          ...
          <CardContent className="space-y-4">
                ...
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                  <div className="bg-white rounded-lg p-6 text-center space-y-4">
                    ...
                    <div className="text-sm text-slate-600">
                      {Math.round(effectsProgress)}%
                    </div>
                    <div className="text-xs text-slate-500">
                      Applying {videoEffect === "blur" ? "blur" : "pixelate"}{" "}
                      effect
            ...
            <div className="flex justify-center gap-4 flex-wrap">
              ...
              <Button
                ...
                disabled={
                  isProcessingEffects || videoEffect === ("none" as VideoEffect)
                }
              >
